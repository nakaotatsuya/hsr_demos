...
from geometry_msgs.msg import Point, PointStamped, Point, Quaternion, PoseArray, Pose
...
class Sosk(MyRobot):
    def __init__(self, name):
        ...
        sub_pose = rospy.Subscriber("/people_pose_estimation_2d/pose", PeoplePoseArray, self.callback, queue_size=1)
    ...
    def callback(self, msg):
        ...
        if lfinger0:
            finger = np.array([lfinger0.position.x, lfinger0.position.y, lfinger0.position.z])
            map_to_camera_coords = self.get_a_to_b("map", "head_rgbd_sensor_rgb_frame")
            map_finger =  map_to_camera_coords.transform_vector(finger)
            a = Pose()
            a.position.x = map_finger[0]
            a.position.y = map_finger[1]
            a.position.z = map_finger[2]
            pub_msg.poses.append(a)
            #print(map_finger)
